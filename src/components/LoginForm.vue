<template>
  ...
  <vee-form :validation-schema="loginSchema" @submit="login">
    <!-- Email -->
    <div class="mb-3">
      <label class="inline-block mb-2">{{ $t("auth.login_form.email") }}</label>
      <vee-field
        type="email"
        name="email"
    ...
    <!-- Password -->
    <div class="mb-3">
      <label class="inline-block mb-2">{{ $t("auth.login_form.password") }}</label>
      <vee-field
        type="password"
        name="password"
        class="block w-full py-1.5 px-3 text-gray-800 border border-gray-300
  ...
  methods: {
    async login(values) {
      ...
      try {
        ...
        this.login_alert_msg = 'Invalid user credentials.';
        return;
      }
